...
import os
from collections import deque  # 履歴バッファ用
...
# 過去状態履歴の長さ (YAMLの[-0.30 ... 0.00] と同じ16)
HISTORY_LENGTH = 16
...
class InferenceNode(Node):
    def __init__(self, model_path):
        ...
        self.env_buffer   = deque(maxlen=HISTORY_LENGTH)
    ...
    def joint_state_callback(self, msg):
        ...
        # 履歴に追加
        self.state_buffer.append(self.latest_joint_state)
        self.env_buffer.append(self.latest_joint_state)

    def publish_action(self):
        # 最新状態未取得 or 履歴不足時はスキップ
        if self.latest_joint_state is None or len(self.state_buffer) < HISTORY_LENGTH:
            return

        # 過去状態シーケンスを作成 (古い順→新しい順)
        states = list(self.state_buffer)
        envs   = list(self.env_buffer)
        # NumPy でフラットに結合して (1, 16*5) ベクトルに
        states_np = np.stack(states, axis=0)  # (16,5)
        envs_np   = np.stack(envs, axis=0)    # (16,5)
        state_flat = states_np.reshape(1, -1)  # (1,80)
        env_flat   = envs_np.reshape(1, -1)    # (1,80)

        # Torch テンソル化
        state_seq = torch.tensor(state_flat, dtype=torch.float32, device=self.device)
        env_seq   = torch.tensor(env_flat,   dtype=torch.float32, device=self.device)

        # モデル入力
        obs = {
            'observation.state':             state_seq,  # shape=(1,80)
            'observation.environment_state': env_seq,    # shape=(1,80)
        }
        ...
        with open(csv_path, "a", newline="") as f:
            ...
            if self.write_header:
                writer.writerow(["timestamp", "action1", "action2", "action3", "action4", "gripper"])
...
def main():
    ...
    parser.add_argument('--model-path', type=str, required=True, help='Path to trained ACTPolicy model directory')
